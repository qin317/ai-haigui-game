...
export function ChatBox({ story }: { story: Story }) {
  ...
  return (
    <div className="grid h-full w-full min-h-0 grid-rows-[minmax(0,1fr)_auto] overflow-hidden rounded-xl border border-slate-700/60 bg-slate-900/50">
      ...
      <form onSubmit={handleSend} className="shrink-0 border-t border-slate-700/60 bg-slate-900/30 p-3 flex gap-3">
        <input
          type="text"
          placeholder="输入你的问题…"
          value={input}
          ...
          disabled={isSending}
          className="flex-1 rounded-lg border border-slate-700/60 bg-slate-800/50 px-4 py-2.5 text-sm text-slate-100 placeholder:text-slate-500 focus:border-indigo-500/60 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
        />
        <button
          type="submit"
          disabled={isSending || !input.trim()}
          className="shrink-0 rounded-lg border border-indigo-500/40 bg-indigo-500/10 px-4 py-2.5 text-sm font-medium text-indigo-100 transition hover:border-indigo-400/50 hover:bg-indigo-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          发送
        </button>
      </form>
